    ...
    public partial class Send2CNC : Form
    {
        ...
        private void generate2SidedProgramOriginal()
        {
            if (TC20Flag == true)
            {
                ...
                ncfile3.Add("O0001 " + progName);               // output file is always number 1
                ncfile3.Add("#531 = " + workShiftTextBox.Text + "     (WORKSHIFT VALUE)");
                ncfile3.Add("#530 = " + OffsetTextBox.Text + "     (OFFSET)");
                ncfile3.Add("G10 P0 Z#531   (SET WORKSHIFT)");
                ...
                ncfile3.Add("M00");
                ncfile3.Add("#529 = [#531+#530]  	(CALCULATE WORKSHIFT BY OFFSET AMOUNT)");
                ncfile3.Add("G10 P0 Z#529           (AND SET NEW WORKSHIFT VALUE)");
        ...
        {
            {
                ...
                DirectoryInfo d = new DirectoryInfo(TC20FolderName);
                FileInfo[] Files = d.GetFiles("*.*"); //Getting Text files
                foreach (FileInfo file in Files)
                {
                    testFileNumber = int.Parse(file.Name.Substring(0, 4));
                ...
                {
                    fileExists = false;
                    using (var form = new GetFileNumber(maxFileNumber.ToString().PadLeft(4, '0')))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            d = new DirectoryInfo(TC20FolderName);
                            ...
                            if (fileExists)
                            {
                                //MessageBox.Show("File Number " + fileNumber + " already exists. Please pick a different number.");
                ...
                if (overWriteFile == true)
                {
                    ...
                    string fileName = ncfile3[1];
                    fileName = fileName.Replace("O0001", fileNumber);

                    // create a writer and open the file
                    TextWriter tw = new StreamWriter(TC20FolderName + fileName + ".nc");
                    //tw.Write("%\r\n");
                    string newString = "";
                    foreach (String s in ncfile3)
                    {
                    ...
                    toolStripStatusLabel.Text = "Saved to TC-20 File Server";
        ...
        */
        ...
        /*private void btnEX110Save_Click(object sender, EventArgs e)
        {
            ...
            DirectoryInfo d = new DirectoryInfo(EX110FolderName);
            FileInfo[] Files = d.GetFiles("*.*"); //Getting Text files
            foreach (FileInfo file in Files)
            {
                testFileNumber = int.Parse(file.Name.Substring(0, 4));
            ...
            do
            {
                ...
                using (var form = new GetFileNumber(maxFileNumber.ToString().PadLeft(4, '0')))
                {
                    ...
                    if (result == DialogResult.OK)
                    {
                        ...
                        d = new DirectoryInfo(EX110FolderName);
                        Files = d.GetFiles("*.*"); //Getting Text files
                        foreach (FileInfo file in Files)
                        {
                            if (file.Name.StartsWith(fileNumber)) fileExists = true;
            ...
            // create a writer and open the file
            TextWriter tw = new StreamWriter(EX110FolderName + fileName + ".nc");
            string newString = "";
            foreach (String s in ncfile3)
            {
                if (s.Contains("O0001"))
            ...
            toolStripStatusLabel.Text = "Saved to EX-110 File Server";
        ...
        */
        ...
        /*private void btnSL30Save_Click(object sender, EventArgs e)
        {
            ...
            DirectoryInfo d = new DirectoryInfo(HaasSL30FolderName);
            FileInfo[] Files = d.GetFiles("*.*"); //Getting Text files
            foreach (FileInfo file in Files)
            {
                testFileNumber = int.Parse(file.Name.Substring(0, 4));
            ...
            do
            {
                ...
                using (var form = new GetFileNumber(maxFileNumber.ToString().PadLeft(4, '0')))
                {
                    ...
                    if (result == DialogResult.OK)
                    {
                        ...
                        d = new DirectoryInfo(HaasSL30FolderName);
                        Files = d.GetFiles("*.*"); //Getting Text files
                        foreach (FileInfo file in Files)
                        {
                            if (file.Name.StartsWith(fileNumber)) fileExists = true;
            ...
            // first extract the program name and number from nc3
            //string fileName = ncfile3[1].Split(new char[] { '(', ')' })[1];
            string fileName = ncfile3[0];
            fileName = fileName.Replace("O0001", fileNumber);

            // create a writer and open the file
            TextWriter tw = new StreamWriter(HaasSL30FolderName + fileName + ".nc");
            tw.Write("%\r\n");
            string newString = "";
            foreach (String s in ncfile3)
            {
            ...
            toolStripStatusLabel.Text = "Saved to SL30 File Server";
        ...
        */
        ...
        private void btnHyundaiSave_Click(object sender, EventArgs e)
        {
            ...
            DirectoryInfo d = new DirectoryInfo(HyundaiFolderName);
            FileInfo[] Files = d.GetFiles("*.*"); //Getting Text files
            foreach (FileInfo file in Files)
            {
                testFileNumber = int.Parse(file.Name.Substring(0, 4));
            ...
            do
            {
                ...
                using (var form = new GetFileNumber(maxFileNumber.ToString().PadLeft(4, '0')))
                {
                    ...
                    if (result == DialogResult.OK)
                    {
                        ...
                        d = new DirectoryInfo(HyundaiFolderName);
                        Files = d.GetFiles("*.*"); //Getting Text files
                        foreach (FileInfo file in Files)
                        {
                            if (file.Name.StartsWith(fileNumber)) fileExists = true;
            ...
            // create a writer and open the file
            TextWriter tw = new StreamWriter(HyundaiFolderName + fileName + ".nc");
            tw.Write("%\r\n");
            string newString = "";
            foreach (String s in ncfile3)
            {
            ...
            toolStripStatusLabel.Text = "Saved to Hyundai File Server";
